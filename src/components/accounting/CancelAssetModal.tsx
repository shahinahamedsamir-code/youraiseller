...
import { formatBdt, type AccountingAsset } from "@/lib/accounting-store";
...
export function CancelAssetModal({ asset, open, onClose, onConfirm, saving = false, error }: Props) {
  useEffect(() => {
    if (!open) return;
  ...
  const saleCount = asset.sales?.length ?? 0;
      ...
      <div className="relative w-full max-w-md overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50 px-5 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-slate-900">Cancel Asset</h2>
        ...
        <div className="space-y-4 p-5">
          <p className="text-sm text-slate-600">
            Cancel <span className="font-bold text-slate-800">{asset.name}</span> (
            {formatBdt(asset.purchaseValue)})? This will:
          </p>
          <ul className="list-inside list-disc space-y-1 text-sm text-slate-600">
            <li>Mark the asset as cancelled</li>
            {asset.expenseId && <li>Reverse the purchase payment from your account</li>}
            {saleCount > 0 && <li>Reverse {saleCount} sale record{saleCount > 1 ? "s" : ""}</li>}
          </ul>
          {error && <p className="rounded-lg bg-rose-50 px-3 py-2 text-sm font-medium text-rose-700">{error}</p>}
          <div className="flex gap-2 pt-1">
            <button type="button" disabled={saving} onClick={onConfirm} className="flex-1 rounded-xl bg-rose-600 py-2.5 text-sm font-bold text-white hover:bg-rose-700 disabled:opacity-60">
              {saving ? "Cancelling…" : "Yes, Cancel Asset"}
            </button>
