...
export type AutoCallAdminRechargeMode = "taka" | "minutes";

type Props = {
  open: boolean;
  mode: AutoCallAdminRechargeMode;
  seller: SellerAutoCallSummary | null;
...
export function DevAdminAutoCallRechargeModal({
  open,
  mode,
  seller,
  callPriceTaka,
  ...
  const [value, setValue] = useState(mode === "taka" ? 500 : 30);
  ...
  const minutesPreview = useMemo(() => {
    ...
    if (taka <= 0 || callPriceTaka <= 0) return 0;
    return Math.floor(taka / callPriceTaka);
  }, [mode, value, callPriceTaka]);
          ...
          <div className="flex items-center gap-3">
            <div
              className={clsx(
                ...
                isTaka ? "bg-teal-500/20 text-teal-300" : "bg-violet-500/20 text-violet-300"
              )}
            >
            ...
            <p className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-400">
              <span>
                ...
                <strong className="text-violet-300">
                  {formatAutoCallBdt(seller.balanceTaka)}
                </strong>
              </span>
              <span>
                Loaded:{" "}
                <strong className="text-white">
                  {formatAutoCallBdt(seller.walletTaka)}
                </strong>
              </span>
            </p>
          </div>
          ...
          <div className="flex gap-2 pt-1">
            ...
            <button
              ...
              className={clsx(
                ...
                isTaka ? "bg-teal-600 hover:bg-teal-500" : "bg-violet-600 hover:bg-violet-500"
              )}
            >
