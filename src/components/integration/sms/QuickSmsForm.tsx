...
export function QuickSmsFormFields({
  ...
  const fieldClass =
    "w-full rounded-xl border border-slate-200 px-3 text-base outline-none focus:border-teal-400 focus:ring-2 focus:ring-teal-100 disabled:opacity-60 sm:text-sm";
  const touchFieldClass = clsx(fieldClass, compact ? "py-2.5" : "py-3 sm:py-2.5");

  return (
    <div
      className={clsx(
        compact ? "space-y-3" : "space-y-4 lg:grid lg:grid-cols-2 lg:items-start lg:gap-6 lg:space-y-0"
      )}
    >
      {!hideInlineFeedback && feedback ? (
        <div
          className={clsx(
            "flex items-start gap-2 rounded-xl px-4 py-3 text-sm font-medium lg:col-span-2",
            feedback.type === "ok"
      ...
      {!compact ? (
        <div className="mb-1 flex items-center justify-between gap-2 lg:col-span-2">
          <div>
            <h2 className="text-lg font-extrabold text-slate-900 sm:text-xl">Quick Send</h2>
            <p className="text-sm text-slate-500">Unlimited send while balance &gt; 0</p>
          </div>
          <button
            type="button"
            onClick={() => reload()}
            className="shrink-0 rounded-lg px-2 py-1 text-xs font-bold text-teal-700 hover:bg-teal-50 hover:underline"
          >
            Refresh
          </button>
        </div>
      ...
      <div className={clsx(compact ? "contents" : "space-y-4")}>
        {compact ? (
          <div className="flex items-center justify-between gap-2 rounded-lg bg-teal-50/80 px-3 py-2.5">
            <p className="text-[11px] font-bold uppercase tracking-wide text-slate-500">
              Balance
            </p>
            <p className="text-sm font-extrabold tabular-nums text-slate-900 sm:text-base">
        ...
        <textarea
          ...
          className={clsx(touchFieldClass, "min-h-[6.5rem] resize-y")}
        />
      </div>

      <div className={clsx(compact ? "contents" : "space-y-4 lg:pt-7")}>
        <label className="block">
          <span className="mb-1.5 block text-xs font-semibold text-slate-500">
            Recipient numbers
          </span>
        ...
        {!systemEnabled ? (
          <p className="rounded-lg bg-rose-50 px-3 py-2.5 text-xs font-semibold text-rose-800 ring-1 ring-rose-200 sm:text-sm">
        ...
        <button
          ...
          onClick={handleSend}
          className={clsx(
            "inline-flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-teal-500 to-cyan-500 px-4 py-3 text-sm font-extrabold text-white shadow-md shadow-teal-200/50 disabled:opacity-60 sm:py-2.5",
            !compact && "lg:w-full"
          )}
        >
          {sending ? (
